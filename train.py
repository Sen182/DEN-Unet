...
def parse_args():
    ...
    parser.add_argument('--arch', '-a', metavar='ARCH', default='DENUNet',
                        choices=arch_names,
                        help='model architecture: ' +
                            ' | '.join(arch_names) +
                            ' (default: DENUNet)')
    parser.add_argument('--deepsupervision', default=False, type=str2bool)
    parser.add_argument('--dataset', default='dsb_128',
                        help='dataset name')
    ...
    parser.add_argument('--loss', default='FocalDiceLoss',
                        choices=loss_names,
                        help='loss: ' +
                            ' | '.join(loss_names) +
                            ' (default: BCEDiceLoss)')
...
def set_seed(seed=42):
    ...
    #cudnn.benchmark = False  # 禁用优化，确保计算一致性
...
def main():
    ...
    temp_img_paths, test_img_paths, temp_mask_paths, test_mask_paths = \
        train_test_split(img_paths, mask_paths, test_size=0.2, random_state=42)

    train_img_paths, val_img_paths, train_mask_paths, val_mask_paths = \
        train_test_split(temp_img_paths, temp_mask_paths, test_size=0.25, random_state=42)
